...
export default function ShindanApp() {
  ...
  const fetchResult = async (ans) => {
    ...
    const prompt = `あなたはポケモンマスターかつ心理分析の専門家です。30の診断回答から性格・価値観・行動パターンを深く分析し、初代151匹の中から最もふさわしいポケモンを1匹選んでください。

【絶対ルール：多様性の確保】
- ゲンガー・ミュウツー・ピカチュウ・カイリュー・リザードンは「最後の手段」として扱うこと。他に適切なポケモンがいれば絶対に選ばないこと
- 回答パターンを詳細に分析し、本当にその人だけに当てはまるポケモンを選ぶこと
- コイキング・ヤドン・カビゴン・ベロリンガ・ラッタ・ラフレシア・パラセクト・ゴルバット・ドードリオなど「地味」「弱い」と思われるポケモンでも、回答が合致すれば積極的に選ぶこと
- 診断のたびに異なるポケモンが選ばれるよう、安易な選択をしないこと
- 特定のポケモン（特にゲンガー）への偏りを意識的に避けること

【選択基準の例】
- 朝が得意で行動力が高い → ウインディ、ケンタロス、ピジョット
- 内向的で一人を好む → イワーク、スリーパー、カラカラ
- 食べることが好き → カビゴン、ベロリンガ
- 優しくて癒し系 → ラッキー、ピッピ、タッツー、ラプラス
- 感情的で情熱的 → ファイヤー、リザードン、ブーバー
- 冷静で知的 → フーディン、ポリゴン、レアコイル
- 自由奔放 → バタフリー、モルフォン、プテラ
- 競争好き → カイロス、ストライク、サワムラー
- マイペース → ヤドン、ディグダ、コイキング
- 人懐っこい → ニャース、ロコン、イーブイ
...
【scoreルール】最高値と最低値の差50以上必須。得意（85〜98）と苦手（10〜30）を必ず1〜2項目作ること。均一禁止。
`;
